...
namespace PokemonGo.RocketAPI.Console
{
    internal class Program
    {
        ...
        private static long OriginalExperience = 0;
        private static int TotalPokemon = 0;
        private static int initTimers = 0;
        private static DateTime TimeStarted = DateTime.Now;
        public static DateTime InitSessionDateTime = DateTime.Now;
        public static List<PokemonFamily> CandyList;
        ...
        {
            var doNotEvolveMe = new[]
            {
                // Don't evolve Eevees, we want to deal with them ourselves
                PokemonId.Eevee,

                // Don't evolve the evolved forms of leveling pokemon either
                PokemonId.Kakuna,
                PokemonId.Beedrill,
                PokemonId.Metapod,
            ...
            var pokeCandidates = pokemonDatas.Where(p => p != null).
                Where(p => !doNotEvolveMe.Contains(p.PokemonId))
                .OrderByDescending(p => p.Cp);

            ColoredConsoleWrite(ConsoleColor.DarkGray, $"[{DateTime.Now.ToString("HH:mm:ss")}] Beginning Evolution List..");

            var failedTypes = new List<PokemonId>();

            foreach (var family in Enum.GetValues(typeof(PokemonFamilyId)).Cast<PokemonFamilyId>())
            {
                var pokeFamilyToEvolve =
                    pokeCandidates.Where(p => ReferenceData.EvolutionData[p.PokemonId].PokemonFamily == family).OrderByDescending(p => p.Cp / ReferenceData.EvolutionData[p.PokemonId].CPMultiplier);

                foreach (var pokemon in pokeFamilyToEvolve)
                {
                    if (failedTypes.Contains(pokemon.PokemonId))
                    {
                        continue;
                    }

                    var pokeRef = ReferenceData.EvolutionData[pokemon.PokemonId];
                    var familyCandy = CandyList.First(p => p.FamilyId == pokeRef.PokemonFamily).Candy;
                    var pokeFamilyCandyLimit =
                        ReferenceData.EvolutionData.Where(p => p.Value.PokemonFamily == pokeRef.PokemonFamily).Sum(x => x.Value.CandyToEvolve);

                    if (pokeRef.CandyToEvolve == 0 || pokeRef.CandyToEvolve > familyCandy)
                    {
                        failedTypes.Add(pokemon.PokemonId);
                        continue;
                    }

                    if (pokeRef.EvolutionLevel == 1 && familyCandy < pokeFamilyCandyLimit)
                    {

                    }

                    ColoredConsoleWrite(ConsoleColor.DarkGray, $"[{DateTime.Now.ToString("HH:mm:ss")}] Trying to evolve {pokemon.PokemonId}..");
                    /*
                    ...
                    EvolvePokemonOut evolvePokemonOutProto = await EvolveAPokemon(client, pokemon);
                    if (evolvePokemonOutProto.Result == 1)
                    {
                        countOfEvolvedUnits++;
                        xpCount += evolvePokemonOutProto.ExpAwarded;
                    }
                    else if (evolvePokemonOutProto.Result >= 3)
                    {
                        evolved = false;
                        failedTypes.Add(pokemon.PokemonId);
                    }
        ...
        private static async Task<EvolvePokemonOut> EvolveAPokemon(Client client, PokemonData pokemon)
        {
            var pokeRef = ReferenceData.EvolutionData[pokemon.PokemonId];
            if (pokeRef.CandyToEvolve == 0)
            {
                return new EvolvePokemonOut { Result = 4, CandyAwarded = 0 };
            }
            if (pokeRef.CandyToEvolve > CandyList.First(p => p.FamilyId == pokeRef.PokemonFamily).Candy)
            {
                return new EvolvePokemonOut {Result = 3, CandyAwarded = 0};
            }
            ...
            if (evolvePokemonOutProto.Result == 1)
            {
                ColoredConsoleWrite(ConsoleColor.Cyan, $"[{DateTime.Now.ToString("HH:mm:ss")}] Evolved {pokemon.PokemonId} successfully for {evolvePokemonOutProto.ExpAwarded}xp");

                var pokeData = ReferenceData.EvolutionData[pokemon.PokemonId];
                var candyToUpdate =
                    CandyList.FirstOrDefault(p => p.FamilyId == pokeData.PokemonFamily);
                if (candyToUpdate != null)
                {
                    candyToUpdate.Candy = candyToUpdate.Candy - pokeData.CandyToEvolve +
                                            evolvePokemonOutProto.CandyAwarded;
                }
            }
        ...
        private static async void Execute(bool runEvolutionFirst = false)
        {
            ...
            try
            {
                ...
                // Commenting these as nothing currently references them.
                ...
                if (initTimers == 0)
                {
                    ...
                    OriginalExperience = stats.First(p => p?.Experience != null).Experience;

                    var titleTimer = new System.Timers.Timer(30000) { AutoReset = true, Enabled = true };
                    titleTimer.Elapsed += delegate { ConsoleLevelTitle(profile.Profile.Username, client); };

                    var levelTimer = new System.Timers.Timer(ClientSettings.LevelTimeInterval * 1000) { AutoReset = true, Enabled = true };
                    levelTimer.Elapsed += delegate { PrintLevel(client); };
                    initTimers = 1;
                }

                await TransferUnwantedPokemon(client);

                if (ClientSettings.EvolveAllGivenPokemons && runEvolutionFirst)
                ...
                await GetCandyList(client);

                ListOwnedPokemon(pokemons);
                ...
                {
                    await EvolveAllGivenPokemons(client, pokemons);
                    ColoredConsoleWrite(ConsoleColor.Red, $"[{DateTime.Now.ToString("HH:mm:ss")}] No nearby useful locations found. Evolving Pokemon...");
                }
                else
                {
            ...
            catch (TaskCanceledException) { ColoredConsoleWrite(ConsoleColor.White, $"[{DateTime.Now.ToString("HH:mm:ss")}] {Language.GetPhrases()["task_canceled_ex"]}"); Execute(); }
            catch (UriFormatException) { ColoredConsoleWrite(ConsoleColor.White, $"[{DateTime.Now.ToString("HH:mm:ss")}] {Language.GetPhrases()["sys_uri_format_ex"]}"); Execute(); }
            catch (ArgumentOutOfRangeException) { ColoredConsoleWrite(ConsoleColor.White, $"[{DateTime.Now.ToString("HH:mm:ss")}] {Language.GetPhrases()["arg_out_of_range_ex"]}"); Execute(); }
            catch (ArgumentNullException) { ColoredConsoleWrite(ConsoleColor.White, $"[{DateTime.Now.ToString("HH:mm:ss")}] {Language.GetPhrases()["arg_null_ref"]}"); Execute(); }
            catch (NullReferenceException) { ColoredConsoleWrite(ConsoleColor.White, $"[{DateTime.Now.ToString("HH:mm:ss")}] {Language.GetPhrases()["null_ref"]}"); Execute(); }
        ...
        private static async Task GetCandyList(Client client)
        {
            ColoredConsoleWrite(ConsoleColor.DarkGreen, $"[{DateTime.Now.ToString("HH:mm:ss")}] Getting candy for pokemon families!");
            var inventory = await client.GetInventory();
            var pokeFamilies = inventory.InventoryDelta.InventoryItems.Select(i => i.InventoryItemData?.PokemonFamily)
                    .Where(p => p != null).ToList();
            CandyList = pokeFamilies;
        }
        ...
        private static async Task ExecuteCatchAllNearbyPokemons(Client client)
        {
            ...
            ColoredConsoleWrite(ConsoleColor.White, $"[{DateTime.Now.ToString("HH:mm:ss")}] Going Pokemon Hunting!");
            ...
            foreach (var pokemon in pokemons)
            {
                ...
                if (caughtPokemonResponse.Status == CatchPokemonResponse.Types.CatchStatus.CatchSuccess)
                {
                    ...
                    var caughtPokemon = encounterPokemonResponse?.WildPokemon?.PokemonData;
                    if (caughtPokemon != null)
                    {
                        var inventory = await client.GetInventory();
                        var pokemonToProcess = inventory.InventoryDelta.InventoryItems
                            .Select(i => i.InventoryItemData?.Pokemon)
                            .First(p => p != null && p.PokemonId == caughtPokemon.PokemonId && p.Cp == caughtPokemon.Cp 
                                && p.IndividualDefense == caughtPokemon.IndividualDefense);
                        await HandleCaughtPokemon(client, pokemonToProcess);
                    }
        ...
        private static async Task HandleCaughtPokemon(Client client, PokemonData pokemon)
        {
            TotalPokemon++;
            var pokeFamilyData =
                CandyList.FirstOrDefault(p => p.FamilyId == ReferenceData.EvolutionData[pokemon.PokemonId].PokemonFamily);
            if (pokeFamilyData != null)
            {
                pokeFamilyData.Candy = pokeFamilyData.Candy + 3;
            }
            ...
            if (ClientSettings.EvolveAllGivenPokemons)
            {
                ...
                if (levelingPokemon.Contains(pokemon.PokemonId))
                {
                    var evolvePokemonOutProto =  await EvolveAPokemon(client, pokemon);
                    if (evolvePokemonOutProto.Result != 1)
                    {
                        // TODO: Convert status code to enum
                        ColoredConsoleWrite(ConsoleColor.Cyan, $"[{DateTime.Now.ToString("HH:mm:ss")}] Failed to evolve {pokemon.PokemonId}. Got status code {evolvePokemonOutProto.Result}");
                    }
                }
            }
            ...
            switch (ClientSettings.TransferType)
            {
                ...
                //    await TransferAllGivenPokemons(client, pokemons);
                //    break;
                // Fix this once you have broken out transfer for individual pokemon
                case "duplicate":
                    await TransferDuplicatePokemon(client);
                    break;
                case "cp":
        ...
        private static async Task ExecuteFarmingPokestopsAndPokemons(Client client)
        {
            ...
            ColoredConsoleWrite(ConsoleColor.White, $"[{DateTime.Now.ToString("HH:mm:ss")}] Checking Nearby Pokestops");
        ...
        private static void ListOwnedPokemon(IEnumerable<PokemonData> pokemonData)
        {
            var pokemons = pokemonData.OrderBy(p => p.Cp).ToList();
            ColoredConsoleWrite(ConsoleColor.White, $"You have {pokemons.Count} Pokemon:");

            foreach (var pokemon in pokemons)
            {
                ColoredConsoleWrite(ConsoleColor.DarkCyan, $"You have {pokemon.PokemonId} at {pokemon.Cp}.");
            }
        }

        private static async Task TransferUnwantedPokemon(Client client)
        {
        ...
        private static async Task TransferAllButStrongestUnwantedPokemon(Client client)
        {
            ...
            foreach (var unwantedPokemonType in unwantedPokemonTypes)
            {
                ...
                List<PokemonData> unwantedPokemon;

                if (unwantedPokemonType == PokemonId.Eevee)
                {
                    unwantedPokemon =
                    pokemonOfDesiredType.Skip(3) // keep the strongest several to enable better abusing the named eevolutions
                        .ToList();
                }
                else
                {
        ...
        private static async Task TransferAllGivenPokemons(Client client, IEnumerable<PokemonData> unwantedPokemons, float keepPerfectPokemonLimit = 80.0f)
        {
            ...
            {
                ColoredConsoleWrite(ConsoleColor.White, $"[{DateTime.Now.ToString("HH:mm:ss")}] Looking to transfer [pokemon].".Replace("[pokemon]", Language.GetPokemons()[Convert.ToString(pokemon.PokemonId)]));
                ...
                if (pokemon.Favorite == 0)
                {
                    ...
                    if (transferPokemonResponse.Status == 1)
                    {
                        ...
                        var pokeFamilyToUpdate = ReferenceData.EvolutionData[pokemon.PokemonId].PokemonFamily;
                        var candyEntry = CandyList.FirstOrDefault(p => p.FamilyId == pokeFamilyToUpdate);
                        if (candyEntry != null)
                        {
                            candyEntry.Candy++;
                        }
                ...
                {
                    ColoredConsoleWrite(ConsoleColor.Magenta, $"[{DateTime.Now.ToString("HH:mm:ss")}] Kept Pokemon [pokemon] because it was favorited.".Replace("[pokemon]", Language.GetPokemons()[Convert.ToString(pokemon.PokemonId)]));
                }
            }
        }
        ...
        public static async Task ConsoleLevelTitle(string Username, Client client)
        {
            var inventory = await client.GetInventory();
            var stats = inventory.InventoryDelta.InventoryItems.Select(i => i.InventoryItemData.PlayerStats).ToArray();
            var xpGained = stats.First(p => p?.Experience != null).Experience - OriginalExperience;
            var profile = await client.GetProfile();
            foreach (var v in stats)
                if (v != null)
                {
                    int XpDiff = GetXpDiff(client, v.Level);
                    System.Console.Title = string.Format(Username + " | Level: {0:0} - ({1:0} / {2:0}) | Stardust: {3:0}", v.Level, (v.Experience - v.PrevLevelXp - XpDiff), (v.NextLevelXp - v.PrevLevelXp - XpDiff), profile.Profile.Currency.ToArray()[1].Amount) + " | XP/Hour: " + Math.Round(xpGained / GetRuntime()) + " | Pokemon/Hour: " + Math.Round(TotalPokemon / GetRuntime()) + " | Lat/Lng: " + Convert.ToString(client.GetCurrentLat()) + "/" + Convert.ToString(client.GetCurrentLng());
                }
